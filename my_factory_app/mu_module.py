...
def _admin_required(view):
    ...
    def wrapped(*args, **kwargs):
        if not session.get("admin_logged_in") or session.get("admin_role") != "admin_mu":
...
def change_employee_password(emp_id):
    ...
    password = request.form.get("password", "")
    if len(password) < 8:
        flash("รหัสผ่านหน้าเบิกต้องมีอย่างน้อย 8 ตัวอักษร", "danger")
...
def update_admin_profile():
    ...
    try:
        ...
        if not admin or admin["role"] != "admin_mu" or not check_password_hash(admin["password"], current_password):
